...
finalres = pd.DataFrame( columns = ['loopvarname', 'loopvar','strategy','races','bets','profit', 'profitability', 'hits', 'hits_pct', 'odds_min' , 'odds_avg', 'odds_max' ] )

'''
np.min(df.algodata['NumDogs'])
np.max(df.algodata['NumDogs'])
round(np.average(df.algodata['NumDogs']),2)
'''

#df.algodata.NumDogs.value_counts(dropna = False)

#dog_count_list = df.algodata.NumDogs.unique().tolist()
#for dogcount in dog_count_list:
#    df.loopdata = df.algodata[df.algodata.NumDogs_bin.isin( [dogcount] )]
#    loopvarname = str('dogcount_bin')
    
dog_count_bin = df.algodata.NumDogs_bin.unique().tolist()
for loopvar in dog_count_bin:
    df.loopdata = df.algodata[df.algodata.NumDogs_bin.isin( [loopvar] )]    
    loopvarname = str('dogcount_bin')
    
    for strat in strategies_1:
        df.loopdata.loc[:,'p'+ strat] = df.loopdata.apply(lambda x : x.BSP_WIN - 1 if ( x['s'+strat] == x['flag_win'] ) & (x['s'+strat])\
                                            else -1 if ~( x['s'+strat] == x['flag_win'] ) & (x['s'+strat]) \
                                                else 0
                                        , axis=1)
        df.loopdata.loc[:,'hit'+ strat] = df.loopdata.apply(lambda x : 1 if ( x['s'+strat] == x['flag_win'] ) & (x['s'+strat]) else 0, axis=1)
        
        df.loopdata.loc[:,'odds'+ strat] = df.loopdata.apply(lambda x : x.BSP_WIN if (x['s'+strat]) else None , axis=1)
    
        # Adding the Necessary cols
        races = len(df.loopdata.EVENT_ID_WIN.unique())
        bets = df.loopdata['s'+strat].sum()
        profit = df.loopdata['p'+strat].sum()
        profitability = round(float(profit/ bets)*100,1)
        
        hits = df.loopdata['hit'+strat].sum()
        hit_pct = round(float(hits/ bets)*100,1)
        
        odds_min = round(np.min(df.loopdata['odds'+ strat]),2)
        odds_avg = round(np.mean(df.loopdata['odds'+ strat]),2)
        odds_max = round(np.max(df.loopdata['odds'+ strat]),2)
        
        row_input = [loopvarname, loopvar, strat, races, bets, profit, profitability, hits, hit_pct, odds_min, odds_avg, odds_max]
        
        finalres.loc[len(finalres)] = row_input
        
    
    for strat in strategies_2:
        df.loopdata.loc[:,'p'+ strat] = df.loopdata.apply(lambda x : x.BSP_PLC - 1 if ( x['s'+strat] == x['flag_plc'] ) & (x['s'+strat]) & (x.NumDogs >=5) \
                                            else -1 if ~( x['s'+strat] == x['flag_plc'] ) & (x['s'+strat]) & (x.NumDogs >=5) \
                                                else 0
                                        , axis=1)
        df.loopdata.loc[:,'hit'+ strat] = df.loopdata.apply(lambda x : 1 if ( x['s'+strat] == x['flag_plc'] ) & (x['s'+strat]) & (x.NumDogs >=5) else 0, axis=1)
    
        df.loopdata.loc[:,'odds'+ strat] = df.loopdata.apply(lambda x : x.BSP_PLC if (x['s'+strat]) else None , axis=1)
